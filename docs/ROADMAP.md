...
| Item | Size | Status | Depends on | Done means |
|---|---|---|---|---|
| Phase 0: repo + media audit | M | SHIPPED 2026-07-24 | — | Read-only audit of the repo, all 8 original videos, 10 photos, and the song list; route conflict surfaced (main site can't host this yet) and the proposals-site alternative approved by Adrian. |
| Phase 1 / build 01: page system, posters, cards, reel | L | BUILT 2026-07-24, committed `8762b1d`, local only | — | Three generated pages (`/adrian`, `/solo`, `/vocalist`), responsive posters + heroes, two email cards, reel v4 (2:21) from Adrian's own thirteen clip windows, all video self-hosted (no Vimeo), players verified playing and seeking at desktop + mobile. Nothing deployed, nothing pushed. See D16. |
| Build 02: Adrian's reconsiderations + ChatGPT audit findings | ? | NEXT, scoping needed | Adrian's list + his ChatGPT review of `~/Desktop/adrian-epk-audit-2026-07-24.zip` | Open at session start: he said there are things to reconsider but wanted a fresh session. Known candidates already on the table: reel length (2:21 may be long for cold outreach), more photos he has available (a wide horizontal hero and a restaurant/lounge shot for the solo lane were requested), the testimonial pull quote. |
| Phase 2: draft deploy + preview verification | S | BLOCKED on build 02 | Adrian's go | Regenerate `dist/`, copy pages + `~/Desktop/EPK/renders/web/*.mp4` into the proposals folder, `netlify deploy` WITHOUT `--prod`, verify every player and link on the draft URL. Sequencing hazard: a proposals deploy publishes the whole folder, so check that repo's git status first. |
| Phase 3: production | S | BLOCKED on Phase 2 | Adrian's explicit production approval | Live at `proposals.greenwayband.com/adrian`, smoke-tested, rollback deploy id recorded. Still `noindex` until he separately approves indexing. |
