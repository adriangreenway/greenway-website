...
| Item | Size | Status | Depends on | Done means |
|---|---|---|---|---|
| Proposal template upgrade: photos + embedded video + CTA | M | SHIPPED 2026-07-23 | — | Built, locally verified (see spec below + CURRENT_STATE.md). Applies to the next new proposal build automatically; no live proposal redeployed yet. |
...
| Template v7+v8: strict-spec rollback + 10pc-first comparison grid | M | SHIPPED 2026-07-24 | — | v7: original per-package itemization + Schedule-a-Call-primary closing restored, subtitles → one benefit sentence. v8: 10-Piece always leads Recommended, desktop side-by-side comparison cards, video section trimmed to label + player + caption. Adrian approved the draft ("this is great"). See D14 v7/v8. |
...
- **Verify:** grep tokens = 0, all links checked, desktop + phone-width render pass, no live proposal redeployed (existing pages untouched until their next natural revision).
