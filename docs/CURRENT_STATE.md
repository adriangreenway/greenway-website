...
- 2026-07-24 **Song list linked from proposals** (D15 v6): a bare "See the full song list" link now sits under the video caption in `docs/proposals/TEMPLATE.html`, opens `proposals.greenwayband.com/song-list` in a new tab so the client's proposal stays open behind it, hidden in print (screen-only, matches the video caption's own print/screen split). Template-only change — **existing live proposals (hinojosa, campbell, etc.) are separate generated copies and are untouched**; the link takes effect on the next proposal built from the template. The EPK already had its own link (`docs/adrian-epk/build.py`) from build 01; Squarespace was ruled out (page pulled per Adrian). Verified in an isolated preview: link renders, hover state and print `display:none` both confirmed via computed styles, mobile checked, console clean.
