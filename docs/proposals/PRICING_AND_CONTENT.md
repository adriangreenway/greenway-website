...
| Photo band 2 (before testimonials) | `assets/crowd-2026a.webp` + `assets/greenhouse-walkin-2026a.webp` | Crowd: `.../_JHZ1145.webp`, re-encoded 1600px/q80. Greenhouse: Adrian's own gig photo (real, confirmed his shoot 2026-07-24), re-encoded 1600px/q80 | NEW 2026-07-24 (D14 v11, Adrian's own pairing — "put the couple walking next to the crowd photo, since they're from the same wedding"): on screen the two sit side by side (`.photo-duo`, greenhouse left, crowd right), stacked on mobile, same 52vh/300-480px height cap as every photo band. Print keeps the ORIGINAL single crowd-only band unchanged (a hidden `.photo-band--print-only` twin) — the pairing is screen-only, the 8-page PDF is untouched. `object-position: 50% 30%` on the crowd photo is unchanged; a camera operator is visible in it at this width regardless of position tested (not a new regression — present in the original single-band crop too, just more noticeable at half width; a shorter test height didn't help and was reverted). |
| Photo band 3 (right before closing) | `assets/sparkler-exit-2026a.webp` | Adrian's own gig photo (real, unnamed couple, confirmed his shoot 2026-07-24), re-encoded 1600px/q80 | Sparkler exit, the emotional peak of the night; `object-position: 50% 40%` keeps both faces + sparklers in frame at any width (verified 1280px/980px). NEW 2026-07-24 (D14 v10) — Adrian's own idea, made a template default after a devil's-advocate discussion on whether non-band photos belong on a proposal at all; landed on exactly one, placed as the page's closing beat, not a second "party photo" alongside the crowd shot. |
...
| Unplaced candidate | `public/images/experience/IMG_0668.webp` | — | Not used in the template pool: reviewed poster candidate, but the square 1:1 crop hid the band behind a bride close-up. Available for a future placement if Adrian wants it, no current use. |
| Unplaced candidate (2026-07-24 photo review) | color version of the sparkler-exit couple, same night | Adrian's gig photo | Same moment as photo band 3, in color. Not used — it's the identical exit seconds apart; using both would repeat the same beat. Parked for the website's Experience/Reviews page, paired with this couple's real review, when the Astro rebuild resumes. |
