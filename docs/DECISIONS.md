...
### D14 v10 amendment: a third photo band, real sparkler exit, right before the closing
- **Date:** 2026-07-24 (Adrian shared 3 real gig photos and asked whether any belonged on proposals; a devil's-advocate exchange settled the scope)
- **Why:** Adrian's instinct — "it's important to show beautiful weddings when selling, even if the band isn't on there" — is real, but the counter-case held too: photos of the venue/photographer's work without the band or the party sell someone else's product, and the page's two existing photo bands already do specific proof jobs (band on stage, packed floor). The resolution: the page's story stops right after the crowd shot and the reviews, with no visual ending. A sparkler exit — the actual last moment of a wedding night — fills exactly that gap and nothing else. Of the 3 photos supplied, 2 were the same couple's same exit seconds apart (one B&W, one color) and 1 showed a different wedding with a videographer visible mid-frame and no band in view.
- **Consequences:** exactly one new shared default asset, `assets/sparkler-exit-2026a.webp` (the black-and-white version — Adrian confirmed all 3 are his own real gigs, contract-reuse clause applies same as photo bands 1-2, couple stays unnamed). Placed as its own photo-band-3, directly before `<section class="closing">`, no divider either side (matches the cover→photo-band-1 pattern) — the gradient itself is the transition. Gradient is photo-band-1's exact reverse (cream fading in at top, dark fading out at bottom, since this band sits between the light body and the dark closing rather than the other way round). Height/crop reuses the crowd photo's existing numbers (52vh/300-480px capped) rather than band-1's whole-frame treatment — no instrument at risk of being cropped out here. `object-position: 50% 40%` verified via a throwaway isolated test page (the Browser pane's screenshot tool doesn't reflect scroll position on this page — a known quirk — so verification used a minimal standalone harness instead) at two widths; both faces and the sparklers stayed in frame. Explicitly screen-only: `display: none` in `@media print`, no `-print.jpg` twin, so the 8-page PDF layout is completely unaffected. The color duplicate and the greenhouse photo were NOT added to proposals — parked as candidates for the website's Experience/Reviews page (color one, paired with this couple's real review) once the Astro rebuild resumes; greenhouse one undecided. Applied to TEMPLATE.html (new default for every future proposal) and the hinojosa live page.
- **Reconsider if:** a future proposal's couple or context makes a sparkler-exit photo feel mismatched (e.g., a non-evening or non-Western-format reception), or Adrian wants to swap in a different exit photo later (same filename convention, bump the suffix).

### D14 v11 amendment: the crowd photo gets a partner — greenhouse walk-in, side by side, screen-only
- **Date:** 2026-07-24 (same session — Adrian revisited the greenhouse photo he'd earlier agreed to leave out, with a specific pairing idea)
- **Why:** Adrian: "put the photo of the white couple walking next to the photo of the energetic crowd, since they're from the same wedding... the crowd photo's aspect is a bit off, and having them fill the entire page looks kind of weird too." Read as: pair the greenhouse walk-in (previously parked) with the EXISTING crowd photo band, side by side, rather than two successive full-width bands. Testing showed the reported "fills the entire page" feeling was solved by the pairing itself (two full-width bands collapsing into one shared row), not by shrinking the height further — a shorter test height was tried and reverted because it bought nothing (see Consequences).
- **Consequences:** the crowd photo band is now a paired `.photo-duo` (greenhouse left, crowd right) on screen, same 52vh/300-480px height cap already used everywhere, side-by-side ≥900px / stacked below it. The print/PDF layout is completely unchanged: a hidden `.photo-band--print-only` twin carries the exact original single crowd-only markup, and all existing `@media print` rules for `.photo-band--short` (page-break, 3.3in height, opens the testimonial page) still target it — `.photo-duo` is explicitly `display:none` in print. New shared asset: `assets/greenhouse-walkin-2026a.webp`. Verified via an isolated crop-check harness (the long page's Browser-pane screenshot doesn't reflect scroll position — a known quirk) plus full DOM/computed-style checks on the real page at both breakpoints, console clean. One honest finding, not fixed: a camera operator is visible in the crowd photo at this width at every object-position tested (0% to 30%) — this is NOT a new regression, the same visibility exists in the original single-band crop at this same height; the docs' claim it's hidden was apparently only ever true on narrower/mobile crops. Applied to TEMPLATE.html (new default) and hinojosa's live-bound page; not yet committed or deployed to prod.
- **Reconsider if:** Adrian wants the crowd photo's crop genuinely fixed (would need a different source photo or a horizontal-pan crop, not just vertical object-position), or a future proposal's two paired photos aren't actually the same wedding and that matters to him for a specific client.

### D15: Client song list becomes a hosted, generated page; first green brand accent
- **Date:** 2026-07-24
- **Why:** Adrian wants the client song list usable in proposals and emails as a clean premium page ("white background with the green"), hosted now rather than waiting for the site build. Checked every current Greenway material for "the green we've been using" — site tokens, proposal template, old proposals, live Squarespace, lead-form embed, gig sheets, Growth Hour: none exists (brand-guide teal is the sister brand's, banned here). So the green was chosen fresh: deep forest, hue ~150, deliberately far from teal.
- **Consequences:** new asset on the proposals track in `docs/song-list/`: `Greenway_Client_Song_List_v4.xlsx` (source of truth, from `~/Desktop`) + `build.py` generator → `index.html` (Bodoni Moda / Plus Jakarta Sans; cream `#F5F2ED` wordmark hero on green `#1D392B→#162D21`; paper `#FBFAF8` body; accents `#27503A`/`#57705F`) + `songs.json` for later create-proposal / Growth Hour integration. All 433 songs and every line of page copy come verbatim from the xlsx; the subtitle count is computed from the rows, never typed. Page has genre chips + live search, print styles, no fabricated content. Staged (not deployed) at `~/Desktop/greenway-proposals/song-list/` → `proposals.greenwayband.com/song-list` on Adrian's go. `greenwayband.com/song-list` is a later Squarespace step (link or iframe to the hosted page, never a raw HTML paste) and sits under the D13 go-live conversation.
- **Reconsider if:** Adrian wants a different green (one `:root` swap in `build.py`, rerun), the xlsx gets a v5 (rerun `build.py` with it), or proposals should surface songs inline (consume `songs.json`).

### D15 v2 amendment: green rejected same day; rebuilt verbatim on the proposal motif (charcoal cover over cream)
- **Date:** 2026-07-24 (hours after v1)
- **Why:** Adrian on v1: "I don't know why you made it green. Our color scheme isn't green… superfluous… this seems like it's just Claude slop." The 433-count subtitle and the "set lists are tailored" note were cut as noise. He invoked ultracode; a 16-agent workflow extracted the template's exact tokens and idioms, built 4 candidates (print-editorial, charcoal-cover, minimal-index, two-column-rail), judged them on motif fidelity / owner restraint / 433-song usability against live hinojosa captures, and adversarially audited the winner (2 rounds: token audit, copy audit, responsive/print/JS audit).
- **Consequences:** winner = **charcoal-cover**: the proposal cover reproduced verbatim (`#0A0A09` band, wordmark CSS, 0.5px hairline rules) over a cream body using the template's inverted-variable idiom; two-column hairline song rows, title left / artist right (the investment-list pattern); small-caps letterspaced genre index + underline-only search in a sticky cream bar; print collapses to ink-friendly 2-column pages. Page copy is now ONLY: wordmark, "Song List", genre names, songs, search placeholder, "No matches.", footer links — no counts anywhere. The shell moved to `docs/song-list/template.html` (`build.py` fills `@@NAV@@`/`@@SECTIONS@@`); typographic apostrophes come from the generator. All green values retired permanently.
- **Reconsider if:** Adrian reacts to the charcoal cover wanting the all-cream variant (print-editorial scored a close second and can be rebuilt cheaply), or any future collateral tempts a new off-system color — it must not (see memory `design_motif_discipline`).
